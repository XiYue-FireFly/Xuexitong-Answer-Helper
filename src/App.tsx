...
import { Bell, BookOpen, Brain, Database, History, Minimize2, Settings, Square, Terminal, X } from 'lucide-react';

type TabId = 'automation' | 'chapter' | 'snapshot' | 'history' | 'settings';
    ...
    <div data-theme={settings.theme} style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg-app)', color: 'var(--text-primary)', overflow: 'hidden' }}>
      ...
      {showNotice && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(3,2,8,0.72)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div className="glass-panel animate-fade-in" style={{ width: 'min(720px, 100%)', maxHeight: '88vh', overflowY: 'auto', borderRadius: 8, padding: 22, background: '#121023' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', marginBottom: 18 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <Bell size={20} style={{ color: 'var(--primary-color)' }} />
                <h2 style={{ color: '#fff', fontSize: '1.05rem' }}>使用公告</h2>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, color: 'var(--text-secondary)', fontSize: '0.82rem', lineHeight: 1.65 }}>
              <section>
                <h3 style={{ color: '#fff', fontSize: '0.9rem', marginBottom: 6 }}>第一步：进入设置</h3>
                <p>打开“设置”，启用“真实 WebView”。启用后才能在内置浏览器里抓取真实网页题目和填入答案。</p>
              </section>

              <section>
                <h3 style={{ color: '#fff', fontSize: '0.9rem', marginBottom: 6 }}>第二步：配置搜题方式</h3>
                ...
                <pre style={{ whiteSpace: 'pre-wrap', background: 'rgba(0,0,0,0.22)', border: '1px solid var(--border-glass)', borderRadius: 8, padding: 12, color: '#e2e8f0', fontSize: '0.75rem', lineHeight: 1.55, marginTop: 8 }}>{`题目：示例选择题题干
A. 选项一
B. 选项二
答案：A
解析：这里可以写解析
...
答案：填空答案`}</pre>
              ...
              <section>
                <h3 style={{ color: '#fff', fontSize: '0.9rem', marginBottom: 6 }}>第三步：开始答题</h3>
